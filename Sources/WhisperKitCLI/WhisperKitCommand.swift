...
struct WhisperKitCommand {
    static func run(cliArguments: CLIArguments, task: DecodingTask) async throws {
        if cliArguments.stream {
            try await processStream(cliArguments: cliArguments, task: task)
        } else {
            try await process(cliArguments: cliArguments, task: task)
        }
    }

    private static func process(cliArguments: CLIArguments, task: DecodingTask) async throws {
    ...
    private static func processStream(cliArguments: CLIArguments, task: DecodingTask) async throws {
        let computeOptions = ModelComputeOptions(
